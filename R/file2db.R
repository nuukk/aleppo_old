file2db <- function(name,dbname,directory,export_directory,db1,db2,db3,db4,db5)
{
  ...
  df_all$`Page Type`[is.na(df_all$`Page Type`)] <- 'PD'
  ...
  df_all <- df_all %>% select(country,country_tier,date,url=page,clicks,impressions,ctr,position,index_position,product_type,page_type=`Page Type`,year_week,year_month,week,month)
